...
@section('subcontent')
<h2 class="intro-y text-lg font-medium mt-10">Kind List</h2>
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <button class="btn btn-primary shadow-md mr-2">
            <a href="{{'/admin/add-kind'}}">Add New Kind</a>
        </button>
        <div class="dropdown">
        ...
        <table id="myTable" class="table table-report -mt-2">
            ...
            <tbody>
                ...
                <tr class="intro-x">
                    ...
                    <td class="w-40 !py-4">
                        <a href="" class="underline decoration-dotted whitespace-nowrap">{{$kind->id}}</a>
                    </td>
                    <td class="w-40">
                        <span href="" class="font-medium whitespace-nowrap">{{$kind->name}}</span>
                    </td>
                    <td class="w-40">
                    ...
                    <td class="w-40">
                        <span href="" class="font-medium whitespace-nowrap">{{$kind->created_by}}</span>
                    </td>
                    <td class="w-40">
                    ...
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3" href="{{(" /admin/edit-kind/$kind->id")}}">
                                <i data-lucide="check-square" class="w-4 h-4 mr-1"></i> Edit
                            </a>
                            <a class="btn-delete flex items-center text-danger 
                            " kind_id="{{$kind->id}}" href="#" data-tw-toggle="modal"
                                data-tw-target="#delete-confirmation-modal-kind">
...
<div id="delete-confirmation-modal-kind" class="modal" tabindex="-1" aria-hidden="true">
...
<script>
    $(document).ready(function () {
        $('.btn-delete').click(function () {
            var kind_id = $(this).attr('kind_id');
            var url = "{{('delete-kind/')}}";
            $('#delete-confirmation-modal-kind').find('a').attr('href', url + kind_id);
        });
    });
</script>
@endsection